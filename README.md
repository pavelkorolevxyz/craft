...
Каталог интерфейсов показывает компоненты, их состояния и рецепты сборки. Каталог слайдов проверяет раскладки на тексте, коде, схемах, графиках и медиа. [Связная мини-колода](catalog/slides/case-study.html) показывает те же раскладки в законченном рассказе на данных самого репозитория.
...
Каталоги и мини-колода открываются напрямую через `file://`. Для первой сборки откройте [начало работы](catalog/interfaces/getting-started.html), затем посмотрите [законченные страницы](catalog/interfaces/pages.html).
...
Доставка результата задаётся параметром `--delivery`. Значение `local` (по умолчанию) держит все ресурсы в папке проекта, страница работает через `file://` без сети. Значение `web` предназначено для публикации в интернете: шрифты подключаются с Google Fonts, а валидатор допускает https-ресурсы только с белого списка хостов. Локальный проект можно собрать в один файл без папки ресурсов:

```bash
python3 scripts/bundle.py ./output/материал   # создаёт index.single.html со встроенными CSS, скриптами и шрифтами
```

## Композиция и проверка

Для презентации сначала прочитай [выбор слайда и сценария показа](references/slides/selection.md). Там описаны все раскладки, различия между списком, сравнением и схемой, порядок появления данных и основания для выделения элементов.
...
│   ├── *.html             документация по уровням системы
...
Интерфейсный индекс ведёт к Foundation и пяти уровням атомарной системы. В Foundation лежат семь общих правил, на трёх компонентных уровнях 34 компонента. Каждый компонент описан один раз: назначение, живой пример, HTML, варианты, доступность и публичный контракт. `catalog/interfaces/getting-started.html` ведёт от каркаса до проверки, а `catalog/interfaces/pages.html` показывает три законченных сценария. `catalog/interfaces/sheet.html` держит все состояния на одной странице для Chromium, PDF и визуальной регрессии.

Слайдовый каталог содержит 28 исходных слайдов и 20 вариантов раскладки. Пошаговые фрагменты разворачиваются в 61 экранную страницу. Каждая попадает в PDF без анимации. В каталоге остаются 11 канонических фрагментов для композиции новых колод. Все страницы работают через `file://` и используют канонические ассеты.
...
`SKILL.md` задаёт маршрут, не пересказывает справочники. `identity.md` хранит общие правила оформления, `workflow.md` команды и проверку. У слайдов `selection.md` отвечает за смысл формы и кадры, `design-language.md` за оформление, `authoring.md` за механику, `charts.md` читается только для графиков. У интерфейсов `patterns.md` выбирает геометрию, грамматика задаёт оформление, тематические справочники компонентов дают разметку и поведение.

Не повторяй полное правило в соседней ветке, дай ссылку на его источник. Каталог остаётся живым примером, `craft.json` машинным контрактом. Устройство уровней и расширение системы нужны разработчику Craft, не каждой новой странице. `make check` ограничивает общий объём инструкций, чтобы новые исправления не превращались в ещё один слой повторов.

Проверка навыка независимыми агентами описана в [сценарии оценки](tests/agent/README.md). Она запускается отдельно от обычных тестов и использует оплачиваемые или подписочные модели.
...
- `craft-interface.zip` — ассеты, справочники и каталог интерфейсов. Подходит, если нужен только `interface` без CLI.
- `craft-slides.zip` — ассеты, справочники и каталог слайдов. Подходит, если нужна только презентационная поверхность без CLI.
- `craft-complete.zip` — обе поверхности и команды `scaffold.py`, `compose.py`, `check_project.py`. Это основной пакет для создания новых материалов.

Тестовые команды, визуальные эталоны, локальные материалы и артефакты разработки в выпуск не входят. Перед распаковкой проверь архивы:
...
Начало работы с полным пакетом:

```bash
unzip craft-complete.zip
cd craft-complete
python3 scripts/scaffold.py ./output/report --surface interface --title "Отчёт"
python3 scripts/check_project.py ./output/report
```

Пакеты одной поверхности не содержат CLI. После распаковки открой `catalog/interfaces/index.html` или `catalog/slides/index.html`, выбери публичные конструкции и перенеси нужные локальные ассеты в свой автономный материал.
