...
Каталоги и мини-колода открываются напрямую через `file://`. Для первого знакомства начните со [страниц интерфейсов](catalog/interfaces/pages.html): там компоненты собраны в рабочие конструкции с готовой разметкой.
...
Интерфейсный индекс ведёт к Foundation и пяти уровням атомарной системы. Компонент описан на странице своего уровня: назначение, живой пример, HTML, варианты, доступность и публичный контракт. `catalog/interfaces/pages.html` начинает с полного пути от каркаса до проверки и собирает компоненты ещё в три нейтральных сценария. `catalog/interfaces/sheet.html` держит все состояния на одной странице для Chromium, PDF и визуальной регрессии.
