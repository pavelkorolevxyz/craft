...
def check_content() -> None:
    ...
    workflow_contracts = [
        "управление → видимый результат",
        "для каждого клика, выбора и клавиатурной команды",
        "если результат оказался ниже сгиба, перестрой адаптивное взаимодействие",
        "Назначай акцент по функции",
        "Не чередуй красные и нейтральные блоки",
        "если смысл, выбор, состояние и связь данных не меняются, оставь элемент нейтральным",
    ]
    assert all(contract in workflow for contract in workflow_contracts), "рабочий процесс ослабляет проверку видимой обратной связи или функции акцента"
    interface_contracts = [
        "Результат каждого клика, выбора и клавиатурной команды виден",
        "не должно молча менять панель за пределами экрана",
        "скрытый `aria-live` не заменяет видимый результат",
    ]
